<template>
  <div>
    <header class="card-header header-category print-none">
      <h2>Laporan Penjualan</h2>
    </header>

    <aside class="card-header header-category print-none">
      <div class="form-group row m-auto d-flex">
        <label for="inputPassword" class="col-sm-3 col-form-label"
          >Cari Laporan Penjualan...</label
        >
        ...
        <div class="col-sm-8">
          <date-picker
            ...
            format="YYYY-MM-DD"
            range
            placeholder="Pilih Jarak Tanggal"
          ></date-picker>
        </div>
      </div>
      ...
      <div class="text-center row m-3">
        <div class="col m-2">
          <button class="btn btn-secondary m-auto" @click="getProducts">
            <i class="fad fa-search"></i> Cari
          </button>
        </div>
        <!-- <div class="col m-2" v-if="dateSearch">
          <button class="btn btn-default m-auto border" @click="print">
    ...
    <div class="row mt-4 print-none">
      <div class="col-sm-6 m-auto">
        <div class="card card-head bg-success mb-3">
          <div class="card-body text-white">
            <h5 class="card-title">Pendapatan</h5>
            <p class="card-text">{{ sales["commission"] | formatMoney }}</p>
      ...
      <div class="col-sm-6 m-auto">
        <div class="card card-head bg-warning mb-3">
          <div class="card-body text-white">
            ...
            <p class="card-text">{{ sales["debt"] | formatMoney }}</p>
            <small
              >Nominal Yang Harus Di Berikan Kepada Pusat Setiap Minggu</small
            >
          </div>
        </div>
      </div>
        ...
        <div class="row no-gutters card-body">
          ...
          <div class="col-md-6">
            ...
            <button class="btn btn-success" @click="print">
              <i class="fad fa-file"></i> Cetak Laporan
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="invoice-box">
      ...
      <div class="table-responsive">
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <tr v-if="!sales">
              <td colspan="10" class="text-center">Harap Pilih Tanggal</td>
            </tr>
            <tr v-for="(item, index) in sales['sales']" :key="index">
              <td>{{ index + 1 }}</td>
              <td>
              ...
              <td>
                <div>
                  <ul>
                    <li
                      v-for="(sale, index2) in item['sales_item']"
                      :key="index2"
                      class="text-nowrap"
                    >
                      {{ sale["product_name"] }}
                ...
                <div class="text-left">
                  <ul>
                    <li
                      v-for="(sale, index2) in item['sales_item']"
                      :key="index2"
                      class="text-nowrap list-unstyled"
                    >
                      {{
                        (sale["basic_price"] +
                          sale["centralCommission"] +
                          sale["partnerCommission"])
                          | formatMoney
                      }}
                    </li>
                  </ul>
              ...
              <td>
                <div class="text-left">
                  <ul>
                    <li class="text-nowrap list-unstyled">
                      {{ item["total_additional"] | formatMoney }}
                    </li>
                  </ul>
                </div>
              </td>
              ...
              <td>
                <div class="text-left">
                  <ul>
                    <li class="text-nowrap list-unstyled">
                      {{ item["total_commission"] | formatMoney }}
                    </li>
                  </ul>
                </div>
              </td>
          ...
          <tfoot>
            <!-- <tr>
                        ...
                        <td> {{ sale['total_debt'] | formatMoney }}</td>
                        <td> {{ sale['total_commission'] | formatMoney }}</td>
                    </tr> -->
          </tfoot>
        </table>
      </div>
    ...
    <div
      ...
      data-keyboard="false"
      tabindex="-1"
      aria-labelledby="modal-filterLabel"
      aria-hidden="true"
    >
...
export default {
  ...
  methods: {
    ...
    print() {
      document.title =
        "Laporan Penjualan " +
        moment(this.dateSearch[0]).format("Do-MM-YYYY") +
        " " +
        moment(this.dateSearch[1]).format("Do-MM-YYYY");
      window.print();
    ...
    getProducts() {
      ...
      if (this.dateSearch.length > 0) {
        params = `?from=${moment(this.dateSearch[0]).format(
          "Do-MM-YYYY"
        )}&to=${moment(this.dateSearch[1]).format("Do-MM-YYYY")}`;
      }

      return axios
        .get(`${process.env.VUE_APP_BASE_HOST_API}/sales-report${params}`, {
          headers: {
